...
$hero_image    = stilco_override_media_alt(
	stilco_get_media_image_data(
		...
		stilco_get_theme_asset_uri( 'assets/images/new-hero.png' ),
		'Materac Stilco'
	),
...
$trust_labels  = array(
	...
	stilco_get_page_field( 'home_trust_2_label', '10 Lat Gwarancji', $page_id ),
	stilco_get_page_field( 'home_trust_3_label', 'Darmowa Dostawa', $page_id ),
	stilco_get_page_field( 'home_trust_4_label', 'Polska Produkcja', $page_id ),
);
?>
	...
	<div class="absolute bottom-0 left-0 w-full border-t border-white/20 bg-stilco-dark/40 backdrop-blur-md text-white py-4 z-20 hidden md:block">
		<div class="max-w-7xl mx-auto px-6 flex justify-between items-center text-sm font-medium tracking-wide">
			<div class="flex items-center space-x-3">
				...
				<span><?php echo esc_html( $trust_labels[0] ); ?></span>
			</div>
			<div class="flex items-center space-x-3">
				<svg class="w-6 h-6 text-stilco-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
					<path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"></path>
				</svg>
				<span><?php echo esc_html( $trust_labels[1] ); ?></span>
			</div>
			<div class="flex items-center space-x-3">
				<svg class="w-6 h-6 text-stilco-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				<span><?php echo esc_html( $trust_labels[2] ); ?></span>
			</div>
			<div class="flex items-center space-x-3">
				<svg class="w-6 h-6 text-stilco-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				<span><?php echo esc_html( $trust_labels[3] ); ?></span>
			</div>
		</div>
	</div>
</section>
...
<div class="w-full bg-white py-8 flex items-center justify-center">
	<a href="#dlaczego-my" class="animate-bounce p-2 text-stilco-secondary hover:text-stilco-dark transition-colors flex flex-col items-center gap-2" aria-label="Przewiń wdół">
		<span class="text-sm font-medium tracking-widest uppercase"><?php echo esc_html( stilco_get_page_field( 'home_hero_scroll_label', 'Odkryj więcej', $page_id ) ); ?></span>
