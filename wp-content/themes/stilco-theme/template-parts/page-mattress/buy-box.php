...
$gallery_images = stilco_get_mattress_landing_gallery_images( $page_id );
$size_options   = stilco_get_mattress_landing_size_options( $page_id );
$measure_link   = stilco_get_link_data( 'mattress_measure_link_text', 'mattress_measure_link_url', 'Jak zmierzyć łóżko?', '#', $page_id );
$price_label    = stilco_get_page_field( 'mattress_price_label', 'Cena z dostawą', $page_id );
$active_price   = 0;

foreach ( $size_options as $size_option ) {
	if ( ! empty( $size_option['is_active'] ) ) {
		$active_price = (int) $size_option['price'];
		break;
	}
}

if ( ! $active_price && ! empty( $size_options[0]['price'] ) ) {
	$active_price = (int) $size_options[0]['price'];
}
?>
	...
	<div class="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-stretch">
		<div class="relative lg:sticky lg:top-32 h-fit space-y-4 animate-slide-left" id="product-gallery">
			...
			<div class="w-full aspect-square md:aspect-[4/3] rounded-[2rem] overflow-hidden bg-gray-100 shadow-sm border border-gray-100 mb-4 relative cursor-zoom-in group">
				<img id="main-product-image" src="<?php echo esc_url( $gallery_images[0]['url'] ); ?>" alt="<?php echo esc_attr( $gallery_images[0]['alt'] ); ?>" class="w-full h-full object-cover transform transition-transform duration-500 group-hover:scale-105 mix-blend-multiply">
			</div>
			...
			<div class="flex overflow-x-auto space-x-3 pb-4 snap-x no-scrollbar">
				<?php foreach ( $gallery_images as $index => $image ) : ?>
					...
					<button type="button" class="gallery-thumb flex-shrink-0 w-24 h-24 sm:w-28 sm:h-28 aspect-square rounded-2xl overflow-hidden bg-white cursor-pointer border-2 transition-all snap-center <?php echo esc_attr( $active_class ); ?>" data-image="<?php echo esc_url( $image['url'] ); ?>">
		...
		<div class="flex flex-col justify-center animate-slide-right delay-200">
			<div class="flex items-center space-x-2 mb-4">
				...
				<span class="text-sm font-medium text-gray-500 underline decoration-gray-300 underline-offset-4 cursor-pointer hover:text-stilco-dark transition-colors"><?php echo esc_html( stilco_get_page_field( 'mattress_rating_text', '4.9/5 (Czytaj opinie)', $page_id ) ); ?></span>
			</div>

			<h1 class="text-4xl lg:text-5xl font-serif text-stilco-dark font-bold mb-4 tracking-tight"><?php echo esc_html( get_the_title( $page_id ) ?: 'Materac Stilco' ); ?></h1>
			<p class="text-lg text-gray-600 mb-8 font-sans"><?php echo esc_html( stilco_get_page_field( 'mattress_lead', 'Otulająca warstwa Visco i stabilna baza HR w symfonicznym duecie. Grubość 22 cm idealnego komfortu.', $page_id ) ); ?></p>
			...
			<div class="mb-8">
				<div class="flex justify-between items-end mb-4">
					...
					<a href="<?php echo esc_url( $measure_link['url'] ); ?>" class="text-sm text-gray-500 hover:text-stilco-accent transition-colors underline underline-offset-2"><?php echo esc_html( $measure_link['label'] ); ?></a>
				</div>
			...
			<div class="grid grid-cols-2 lg:grid-cols-4 gap-4 pt-6 border-t border-gray-100">
				<div class="flex flex-col items-center justify-center p-6 bg-white rounded-2xl shadow-sm border border-gray-50 text-center hover:-translate-y-1 transition-transform duration-300">
					...
					<span class="text-sm font-bold text-stilco-dark uppercase tracking-wide leading-tight"><?php echo esc_html( stilco_get_page_field( 'mattress_badge_1_title', '100 nocy', $page_id ) ); ?><br><span class="text-xs text-gray-500 font-medium"><?php echo esc_html( stilco_get_page_field( 'mattress_badge_1_note', 'testowych', $page_id ) ); ?></span></span>
				</div>
				<div class="flex flex-col items-center justify-center p-6 bg-white rounded-2xl shadow-sm border border-gray-50 text-center hover:-translate-y-1 transition-transform duration-300">
					...
					<span class="text-sm font-bold text-stilco-dark uppercase tracking-wide leading-tight"><?php echo esc_html( stilco_get_page_field( 'mattress_badge_2_title', '10 lat', $page_id ) ); ?><br><span class="text-xs text-gray-500 font-medium"><?php echo esc_html( stilco_get_page_field( 'mattress_badge_2_note', 'gwarancji', $page_id ) ); ?></span></span>
				</div>
				<div class="flex flex-col items-center justify-center p-6 bg-white rounded-2xl shadow-sm border border-gray-50 text-center hover:-translate-y-1 transition-transform duration-300">
					...
					<span class="text-sm font-bold text-stilco-dark uppercase tracking-wide leading-tight"><?php echo esc_html( stilco_get_page_field( 'mattress_badge_3_title', 'Darmowa', $page_id ) ); ?><br><span class="text-xs text-gray-500 font-medium"><?php echo esc_html( stilco_get_page_field( 'mattress_badge_3_note', 'Dostawa', $page_id ) ); ?></span></span>
				</div>
				<div class="flex flex-col items-center justify-center p-6 bg-white rounded-2xl shadow-sm border border-gray-50 text-center hover:-translate-y-1 transition-transform duration-300">
					...
					<span class="text-sm font-bold text-stilco-dark uppercase tracking-wide leading-tight"><?php echo esc_html( stilco_get_page_field( 'mattress_badge_4_title', 'Polska', $page_id ) ); ?><br><span class="text-xs text-gray-500 font-medium"><?php echo esc_html( stilco_get_page_field( 'mattress_badge_4_note', 'Produkcja', $page_id ) ); ?></span></span>
				</div>
			</div>
		</div>
	</div>
