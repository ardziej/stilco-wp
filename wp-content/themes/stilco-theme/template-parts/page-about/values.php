...
$image   = stilco_override_media_alt(
	stilco_get_media_image_data(
		stilco_get_page_field( 'about_values_image', '', $page_id ),
		stilco_get_theme_asset_uri( 'assets/images/delivery-materac.jpg' ),
		'Szybka dostawa materaca'
	),
	stilco_get_page_field( 'about_values_image_alt', '', $page_id )
);
?>
		...
		<div class="grid grid-cols-1 md:grid-cols-3 gap-8 auto-rows-[280px]">
			<div class="bg-white rounded-3xl p-10 shadow-sm flex flex-col justify-between border border-gray-100 md:col-span-2 relative overflow-hidden group animate-zoom">
				<div class="relative z-10 w-full mb-6">
					...
					<p class="text-gray-600 text-lg max-w-xl leading-relaxed"><?php echo esc_html( stilco_get_page_field( 'about_value_1_text', 'Używamy wyselekcjonowanych i trwalszych pianek kalibrowanych (np HR40 zamiast taniej T25). Obiecujemy zero kompromisów przy dnie łóżka.', $page_id ) ); ?></p>
				</div>
			</div>
			...
			<div class="bg-white rounded-3xl p-10 shadow-sm flex flex-col justify-center border border-gray-100 animate-zoom delay-200">
				...
				<div class="mt-auto">
					...
					<p class="text-gray-600 text-base leading-relaxed"><?php echo esc_html( stilco_get_page_field( 'about_value_3_text', 'Odbiór nawet do 48h dzięki innowacyjnej prasie rolującej paczki przed błyskawicznym wysłaniem.', $page_id ) ); ?></p>
				</div>
			</div>
